...
from memory_bench_platform.loader import (
    load_agent_skill,
    load_all_skills,
    load_benchmark_skill,
    load_memory_skill,
    load_smoke_skill,
)
...
def test_load_all_skills_reads_benchmark_and_agent_manifests(tmp_path: Path):
    ...
    (skills / "memories" / "openviking").mkdir(parents=True)
    ...
    (skills / "memories" / "openviking" / "manifest.yaml").write_text(
        "kind: memory\nid: openviking\nversion: 0.1.0\nversion_policy:\n  default_selection: latest_official_release_tag\n  resolution_order:\n    - user_specified_official_version\n    - latest_official_release_tag\n  allowed_overrides:\n    - user_specified_official_version\n  disallowed_defaults:\n    - dirty_worktree\n  targets:\n    - name: openviking\n      scope: memory_backend\n      version_source: upstream_release_tag\n      upstream: https://example.com/openviking\n  record_runtime_version: true\ningest:\n  benchmark_unit: session\n",
        encoding="utf-8",
    )
    ...
    assert loaded["memories"][0].id == "openviking"
...
def test_load_single_skill_helpers_read_manifest(tmp_path: Path):
    ...
    (skills / "memories" / "openviking" / "manifest.yaml").write_text(memory_manifest, encoding="utf-8")
    ...
    memory = load_memory_skill(skills, "openviking")
    ...
    assert memory.ingest["benchmark_unit"] == "session"
